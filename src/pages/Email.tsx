...
const Email = () => {
  ...
  const getEmailList = async (params) => {
    try {
      ...
      const res = await axios.post(serverURL + '/api/email/list', params);
      const data = res.data;
  ...
  let initData = { page: pageNum, perPage: Number(perPage), kind: kind, searchValue: searchValue };
  ////
  let pageSize = 5;
  ...
  return (
    <>
      ...
      {roles == "all" ? <Breadcrumb pageName="ALL LIST" /> : ''}
      {roles == "send" ? <Breadcrumb pageName="SEND LIST" /> : ''}
      {roles == "receive" ? <Breadcrumb pageName="RECEIVE LIST" /> : ''}
