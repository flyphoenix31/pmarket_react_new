...
import { setClientList, setErrors } from '../store/slice/clientsSlice';
import { isEmpty } from '../config/index.js';
import axios from 'axios';
import ClientDeleteModal from '../pages/ClientDeleteModal';
import { ArrowDownSVG, ClientSVG, EyeSVG, SearchSVG, TrushSVG } from '../components/SVG.js';

const Clients = () => {

  const dispatch = useDispatch();
  const clientList = useSelector((state: any) => state.clients.clientList);
  ...
  const handleAddNew = (event: any) => {
    ...
    navigate('/member/clients/add');
  }

  const handleView = (id: any) => {
    navigate(`/member/clients/edit/${id}`);
  }

  const getClientStatus = (status) => {
    switch (status) {
      case 1:
        return { className: 'text-success bg-success', data: 'Active' };
      case 2:
        return { className: 'text-danger bg-danger', data: 'Inactive' };
      default:
  ...
  return (
    <>
      <Breadcrumb pageName="CONTACTED CLIENT LIST" />
      <ClientDeleteModal preid={preid} is_deleted={is_deleted} setDelete={setDelete} initPage={initPage} />
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        ...
        <div className="max-w-full overflow-x-auto">
          <table className="w-full table-auto">
            ...
            <tbody>
              {
                clientList.map((client: any, clientIndex: any) => {
                  return (
                    <tr key={clientIndex}>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="text-black dark:text-white" dangerouslySetInnerHTML={{ __html: (isEmpty(client.title) ? '' : (titleList[client.title] + '. ')) + client.name }}></p>
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="text-black dark:text-white">{moment(client.created_at).format('DD-MM-YYYY')}</p>
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className={"inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium " + getClientStatus(client.status_id).className}>
                          {getClientStatus(client.status_id).data}
                        </p>
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <div className="flex items-center space-x-3.5">
                          <button className="hover:text-primary" onClick={e => { e.preventDefault(); handleView(client.id); }}>
                            <EyeSVG />
                          </button>
                          <button className="hover:text-primary" onClick={e => { e.preventDefault(); setDelete(true); setPreId(client.id); }}>
                            <TrushSVG />
                          </button>
                        </div>
                      </td>
