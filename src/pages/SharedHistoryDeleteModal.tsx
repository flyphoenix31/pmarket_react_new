...
const SharedhistoryDeleteModal = ({ preid, is_deleted, setDelete, initPage }) => {
    ...
    const handleSave = (event: any) => {
        event.preventDefault();
        axios.post(serverURL + '/api/shared/hdelete', { id: preid })
            .then(res => {
                const data = res.data;
                if(!data.status) {
                    initPage();
...
export default SharedhistoryDeleteModal;