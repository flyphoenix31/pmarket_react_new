...
import { ArrowDownSVG, EyeSVG, JobsSVG, SearchSVG, TrushSVG } from '../components/SVG.js';
import JobsDeleteModal from './JobsDeleteModal.js';
const Jobs = () => {
  ...
  const handleAddNew = (event: any) => {
    ...
    navigate('/member/jobs/add');
  }

  const handleView = (id) => {
    navigate(`/member/jobs/edit/${id}`);
  }
  ...
  const getJobStatus = (status) => {
    switch (status) {
      ...
      case 2:
        return { className: 'text-warning bg-warning', data: 'Inactive' };
      case 3:
        return { className: 'text-danger bg-danger', data: 'Closed' };
      default:
  ...
  return (
    <>
      ...
      <JobsDeleteModal preid={preid} is_deleted={is_deleted} setDelete={setDelete} initPage={initPage} />
            ...
            <tbody>
              {
                jobsList.map((job, jobsIndex) => (
                  <tr key={jobsIndex}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">{moment(job.created_at).format('DD-MM-YYYY')}</p>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      {jobsdelete_flag ?
                        <div className="flex items-center space-x-3.5">
                          ...
                          <button className="hover:text-primary" onClick={e => { e.preventDefault(); setDelete(true); setPreId(job.id); }}>
                            <TrushSVG />
                          </button>
                        </div>
                        :
