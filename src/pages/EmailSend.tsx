...
const EmailSend = () => {
  ...
  const getEmailList = async (params: { page: number; perPage: number; kind: string; searchValue: string; email?: string | null; }) => {
    try {
      console.log("====preparam", params);
      if (isEmpty(params)) {
        params = initData;
      }
      const res = await axios.post(serverURL + '/api/email/slist', params);
      const data = res.data;
