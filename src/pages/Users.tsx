...
import { disUserList, setRoleList } from '../store/slice/usersSlice.js';
import { serverURL, isEmpty } from '../config';
import { ArrowDownSVG, EmailSVG, EyeSVG, HumanSVG, InvoiceSVG, KeySVG, SearchSVG, TrushSVG, UserSVG, WebSVG } from '../components/SVG.js';
...
const Users = () => {
  ...
  const handleAddNew = (event: any) => {
    ...
    navigate('/member/users/add');
  }

  const handleView = (id: any) => {
    navigate(`/member/users/edit/${id}`);
  }

  const handleEmailView = (id: any) => {
    navigate(`/member/users/emailedit/${id}`);
  }

  return (
    <>
      <Breadcrumb pageName="USER LIST" />
      <UserDeleteModal preid={preid} is_deleted={is_deleted} setDelete={setDelete} initPage={initPage} />
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        ...
        <div className="max-w-full overflow-x-auto">
          <table className="w-full table-auto">
            ...
            <tbody>
              {
                userList.map((user: { role_id: { toString: () => any; }; avatar: any; name: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | ReactFragment | ReactPortal | null | undefined; email: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | ReactFragment | ReactPortal | null | undefined; start_date: moment.MomentInput; id: any; }, userIndex: Key | null | undefined) => {
                  const roleIndex = roleList.findIndex((item: { id: { toString: () => any; }; }) => item.id.toString() === user.role_id.toString());
                  return (
                    <tr key={userIndex}>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        {
                          isEmpty(user.avatar) ? '' : (
                            <img src={serverURL + user.avatar} alt="User Image" style={{ width: '56px', height: '56px', borderRadius: '50%' }} />
                          )
                        }
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="text-black dark:text-white">{roleIndex < 0 ? '' : roleList[roleIndex].name}</p>
                      </td>
                      {/* <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">{user.age}</p>
                    </td> */}
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="text-black dark:text-white">{moment(user.start_date).format('DD-MM-YYYY')}</p>
                      </td>
                      {/* <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">${user.salary}</p>
                    </td> */}
                      {/* <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
