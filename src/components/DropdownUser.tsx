      ...
      >
        <span className="hidden text-right lg:block">
          <span className="block text-sm font-medium text-black dark:text-white">
            {/* Thomas Anree */}
            {!isEmpty(props.headerInfo) ? props.headerInfo.userInfo.name : ''}
          </span>
          <span className="block text-xs">
            {/* UX Designer */}
            {!isEmpty(props.headerInfo) && !isEmpty(props.headerInfo.role) ? props.headerInfo.role.name : ''}
          </span>
        </span>

        <span className="h-12 w-12 rounded-full">
          {
            isEmpty(props.headerInfo) || isEmpty(props.headerInfo.userInfo) || isEmpty(props.headerInfo.userInfo.avatar) ? '' : (
              <img src={serverURL + props.headerInfo.userInfo.avatar} alt="User" style={{ borderRadius: '50%' }} />
            )
          }
        </span>
